...
This repo is the playground for the **10xDevs AI Toolkit** course. Foundation artifacts live under `context/foundation/` (e.g. `prd.md`, `tech-stack.md`, `lessons.md`); change records under `context/changes/`; archived changes are immutable in `context/archive/` — **never write to `context/archive/`**, open a new change with `/10x-new` instead.
